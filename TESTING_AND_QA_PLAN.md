...
describe("EventService", () => {
  ...
  describe("getEventsByZone", () => {
    it("should return paginated events", async () => {
      ...
      // Act
      const result = await eventService.getEventsByZone(zoneId, {
        page: 1,
        pageSize: 10,
      });
...
describe("FlowEngineService", () => {
  ...
  describe("executeFlow", () => {
        ...
        actions: [
          {
            ...
            thenActions: [
              { type: "webhook", url: "https://notify.example.com" },
            ],
            elseActions: [],
          },
        ],
    ...
    it("should enforce timeout on action execution", async () => {
      ...
      // Act & Assert
      await expect(
        flowEngine.executeFlow(flow, { id: "evt_123" }),
  ...
  describe("validateFlow", () => {
    it("should validate flow structure", async () => {
      ...
      // Act & Assert
      await expect(flowEngine.validateFlow(invalidFlow)).rejects.toThrow(
        "Invalid action type",
      );
    });
    ...
    it("should catch circular references", async () => {
      ...
      // Act & Assert
      await expect(flowEngine.validateFlow(flow)).rejects.toThrow(
...
describe("Sapliy SDK", () => {
  ...
  describe("emit", () => {
    ...
    it("should validate event type", async () => {
      // Act & Assert
      await expect(sapliy.emit(null as any, {})).rejects.toThrow(
  ...
  describe("retry logic", () => {
    ...
    it("should not retry on permanent failure", async () => {
      ...
      // Act & Assert
      await expect(sapliy.emit("test.event", {})).rejects.toThrow(
        "Unauthorized",
      );
...
describe("sapliy run command", () => {
  ...
  describe("execute", () => {
    it("should start all services", async () => {
      ...
      // Assert
      expect(dockerService.compose).toHaveBeenCalledWith(
        "up",
        expect.any(Object),
      );
    });
    ...
    it("should handle Docker not installed", async () => {
      ...
      // Act & Assert
      await expect(runCommand.execute({})).rejects.toThrow("Docker not found");
    });
  });
});
```
...
describe("Payment Event Flow", () => {
  ...
  it("should process complete payment flow", async () => {
    ...
    const initiatedFlow =
      await flowEngine.getLastExecution("payment.initiated");
